...
def set_dialog():
    ...
    icon = req.get("icon", "")
    permission=req.get("permission","team")
...
def list_dialogs():
    try:
        tenantItems = TenantService.get_joined_tenants_by_user_id(current_user.id)
        tenant_ids = [t["tenant_id"] for t in tenantItems]
        tenant_ids.append(current_user.id)
        ...
        # logging.info('aaaa',diags)
        # 新增过滤逻辑
        filtered_diags = []
        for d in diags:
            # 假设 permission 字段存在于每个 diag 中
            if d["tenant_id"] == current_user.id or d.get("permission") == "team":
                d["kb_ids"], d["kb_names"] = get_kb_names(d["kb_ids"])
                filtered_diags.append(d)
        return get_json_result(data=filtered_diags)
    except Exception as e:
        return server_error_response(e)
