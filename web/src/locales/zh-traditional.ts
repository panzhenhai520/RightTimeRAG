export default {
  translation: {
    ...
    homeDashboard: {
      chatAssistants: '聊天助手',
      chatAssistantsDescription: '選擇已配置好的專業助手，直接開始問答。',
      searchTools: '搜尋助手',
      searchToolsDescription: '進入專門的搜尋工具，按文檔和證據檢索資料。',
      publishedAgents: '已發布智能體',
      publishedAgentsDescription: '運行已發布的智能體，普通用戶不可編輯流程。',
      knowledgeAssets: '知識庫',
      knowledgeAssetsDescription: '查看可用知識資產、文檔數量和最近更新。',
      recentMemos: '最近備忘錄',
    ...
    devSettingPanython: {
      pageTitle: 'Panython 開發管理',
      pageDescription: '這些功能用於後台配置，不在普通用戶工作台展示。',
      tabMenus: '功能入口',
      tabTts: 'TTS 語音',
      tabUsers: '用戶註冊',
      tabTenants: '用戶組設定',
      ...
      createChat: '新建聊天助手',
      createChatDescription: '建立並配置聊天助手、模型、提示詞和關聯知識庫。',
      createSearch: '新建搜尋助手',
      createSearchDescription: '建立並配置搜尋助手、搜尋範圍和結果呈現方式。',
      agents: '智能體',
      agentsDescription: '管理預定義智能體和流程編排。',
      createAgent: '新建智能體',
      createAgentDescription:
        '從模板或空白流程建立智能體，並進入流程編排向導。',
      memory: '記憶整理',
      memoryDescription: '管理記憶、訊息整理和後續備忘錄擴展。',
      files: '文件管理',
      filesDescription: '管理系統文件和技能文件。',
      configuration: '配置管理',
      configurationDescription: '管理資料來源和外部連接配置。',
      tenantRelationsTitle: '用戶組設定',
      tenantRelationsDescription:
        '按用戶組展示成員、聊天助手和助手可訪問知識庫。用戶加入某個用戶組後，就能使用該用戶組下發布的助手；助手可以在這裡維護可訪問知識庫。',
      selectUser: '選擇用戶',
      selectTenant: '選擇用戶組',
      selectUserGroup: '選擇用戶組',
      saveUserTenant: '保存用戶組成員',
      saveUserGroupMembership: '保存用戶組成員',
      tenantRelationSaved: '用戶組成員關係已保存',
      tenantRelationDeleted: '用戶組成員關係已刪除',
      deleteRelationBlocked: '不能刪除該上級關係，仍存在下級資產：{{blockers}}',
      deleteRelationConfirm:
        '確認把 {{user}} 從用戶組 {{tenant}} 中移除？該操作會影響該用戶可見範圍。',
      transferDialogConfirm:
        '確認把聊天助手「{{dialog}}」遷移到用戶組「{{tenant}}」？',
      dialogTenantSaved: '聊天助手所屬用戶組已調整',
      dialogKnowledgeSaved: '聊天助手知識庫已更新',
      updateFailed: '更新失敗',
      unknownTenant: '未知用戶組',
      unnamedTenant: '未命名用戶組',
      unnamedUser: '未命名用戶',
      tenantLabel: '用戶組：{{tenant}}',
      userGroupLabel: '用戶組：{{group}}',
      tenantSummary:
        '{{users}} 個用戶 / {{dialogs}} 個聊天助手 / {{kbs}} 個知識庫',
      usersInTenant: '用戶組成員',
      groupMembers: '用戶組成員',
      noTenantUsers: '暫無用戶加入該用戶組',
      userBelongsToTenant: '{{user}} 屬於用戶組 {{tenant}}',
      removeUserTenant: '移除用戶組成員',
      hasDependentAssets: '仍有下級資產：{{blockers}}',
      tenantKnowledgebases: '該用戶組的知識庫',
      groupKnowledgebases: '該用戶組的知識庫',
      noKnowledgebases: '暫無知識庫',
      kbStats: '{{docs}} 文件 / {{chunks}} 切片 / {{permission}}',
      noDialogsInTenant: '該用戶組下暫無聊天助手。',
      dialogLabel: '聊天助手：{{dialog}}',
      dialogTenantAccess: '用戶組「{{tenant}}」的成員可以使用該助手。',
      groupAssistants: '該用戶組可用的聊天助手',
      moveToTenant: '遷移到其他用戶組',
      saveOwner: '保存所屬用戶組',
      accessibleKnowledgebases: '可訪問知識庫',
      selectedKbCount: '已選 {{count}} 個',
      kbTenantTooltip: '{{kb}} 屬於用戶組 {{tenant}}',
      noBindableKnowledgebases: '暫無可綁定知識庫。',
      kbOptionMeta: '{{tenant}} / {{docs}} 文件 / {{chunks}} 切片',
      saveKnowledgeAccess: '保存知識庫權限',
      assetMembers: '成員',
      assetDatasets: '知識庫',
      assetDialogs: '聊天',
      assetSearches: '搜尋',
      ...
      deleteGroupBlocked:
        '不能刪除該用戶組，下面仍有用戶或資源：{{blockers}}。請先遷移或刪除下級內容。',
      deleteGroupConfirm:
        '確認刪除用戶組「{{tenant}}」？刪除後該用戶組入口不可用，該操作會記錄日誌。',
      userGroupDeleted: '用戶組已刪除',
      deleteDialog: '刪除助手',
      deleteDialogConfirm:
        '確認刪除聊天助手「{{dialog}}」？刪除後用戶將不能繼續使用該助手，相關歷史入口可能不可見。該操作會記錄日誌。',
      dialogDeleted: '聊天助手已刪除',
      operationLogsTitle: '用戶管理操作日誌',
      operationLogsDescription:
        '記錄用戶組、用戶組成員和聊天助手歸屬相關操作。當前共 {{total}} 條。',
      logOperator: '操作人',
      logAction: '操作',
      logTarget: '對象',
      logTime: '時間',
      noOperationLogs: '暫無操作日誌',
      operation_user_group_membership_create: '新增用戶組成員',
      ...
      email: '郵箱',
      password: '密碼',
      ttsEngineTitle: 'TTS 語音引擎',
      ttsEngineDescription:
        '只有在這裡啟用 TTS，並聲明引擎支持對應能力後，聊天助手和智能體才顯示語音參數配置。',
      ttsSaved: 'TTS 引擎配置已保存',
      enableTtsFirst: '請先啟用 TTS 語音能力',
      previewFailed: '試聽生成失敗',
      enableTts: '啟用 TTS 語音能力',
      enableTtsDescription: '關閉後不緩存、不排隊、不延遲文字輸出。',
      engineName: '引擎名稱',
      engineCapabilities: '引擎能力聲明',
      ttsCapabilitySpeed: '語速',
      ttsCapabilityEmotion: '情緒',
      ttsCapabilityDialect: '方言',
      ttsCapabilityVoice: '音色 Profile',
      ttsCapabilitySync: '聲文同步',
      defaultSpeed: '默認語速',
      currentSpeed: '當前 {{speed}}x',
      defaultEmotion: '默認情緒',
      defaultDialect: '默認中文方言',
      defaultGender: '默認性別',
      genderFemale: '女聲',
      genderMale: '男聲',
      defaultVoiceProfile: '默認音色 Profile',
      currentVoiceProfile: '當前：{{profile}}',
      bufferMs: '首段緩衝',
      bufferMsValue: '{{value}} ms',
      bufferMsHelp:
        '首段緩衝控制首段語音開始播放前預先緩存的時長，單位為毫秒。網絡或 TTS 生成較慢、開頭容易卡頓時調大；希望更快開口時調小。',
      zhSegmentChars: '中文分段字數',
      zhSegmentCharsValue: '{{value}} 字',
      zhSegmentCharsHelp:
        '中文分段字數控制中文回答按多少個漢字切成一段送入 TTS。段越短越快開始播放但銜接更頻繁；段越長語氣更連貫但等待更久。',
      enSegmentWords: '英文分段詞數',
      enSegmentWordsValue: '{{value}} 詞',
      enSegmentWordsHelp:
        '英文分段詞數控制英文回答按多少個英文詞切成一段送入 TTS。英文朗讀卡頓時可調小，長句語氣不連貫時可調大。',
      preview: '試聽',
      saveTts: '保存 TTS 配置',
      ttsPreviewText: '歡迎訪問時和專業AI顧問。This is a speech preview.',
      ttsSpeedSlow: '偏慢 0.8x',
      ttsSpeedNormal: '正常 1.0x',
      ttsSpeedSlightlyFast: '稍快 1.15x',
      ttsSpeedFast: '較快 1.3x',
      ttsEmotionProfessional: '專業',
      ttsEmotionCalm: '平靜',
      ttsEmotionFriendly: '親切',
      ttsEmotionFormal: '正式',
      ttsEmotionLively: '活潑',
      ttsEmotionSerious: '嚴肅',
      ttsDialectMandarin: '普通話',
      ttsDialectCantonese: '粵語/廣東話',
      ttsDialectSichuan: '四川',
      ttsDialectShanghai: '上海',
      ttsDialectDongbei: '東北',
      ttsDialectMinnan: '閩南',
      ttsDialectTianjin: '天津',
      ttsDialectShandong: '山東',
      ttsVoiceFemaleMandarin: '女聲 / 普通話 / 標準',
      ttsVoiceMaleMandarin: '男聲 / 普通話 / 標準',
      ttsVoiceFemaleCantonese: '女聲 / 粵語 / 標準',
    ...
    knowledgeDetails: {
      ...
      parsingStatus: '解析狀態',
      parsingStatusTip:
        '文件解析時間取決於多種因素。如果啟用了知識圖譜、RAPTOR、自動問題提取、自動關鍵詞提取等功能，解析時間將會顯著延長。如果解析進度條長時間未更新，請參考這兩條常見問題解答FAQ：https://Panython.io/docs/dev/faq#why-does-my-document-parsing-stall-at-under-one-percent。',
      ...
      layoutRecognizeTip:
        '使用視覺模型進行 PDF 布局分析，以更好地識別文檔結構，找到標題、文字塊、圖像和表格的位置。若選擇 Naive 選項，則只能取得 PDF 的純文字。請注意此功能僅適用於 PDF 文檔，對其他文檔不生效。如需更多資訊，請參閱 https://Panython.io/docs/dev/select_pdf_parser。',
      taskPageSize: '任務頁面大小',
      ...
      uploadDescription:
        '支持單次或批量上傳。本地部署的單次上傳檔案總大小上限為 1GB，單次批量上傳檔案數不超過 32，單個帳戶不限檔案數量。對於 cloud.Panython.io，每次上傳的文件總大小限制為 10MB，每個文件不得超過 10MB，且每個帳戶最多可上傳 128 個文件。',
      chunk: '解析塊',
      bulk: '批量',
      cancel: '取消',
      rerankModel: 'rerank模型',
      rerankPlaceholder: '請選擇',
      rerankTip: `非必選項：若不選擇 rerank 模型，系統將默認採用關鍵詞相似度與向量餘弦相似度相結合的混合查詢方式；如果設定了 rerank 模型，則混合查詢中的向量相似度部分將被 rerank 打分替代。請注意：採用 rerank 模型會非常耗時。如需選用 rerank 模型，建議使用 SaaS 的 rerank 模型服務；如果你傾向使用本地部署的 rerank 模型，請務必確保你使用 docker-compose-gpu.yml 啟動 Panython。`,
      topK: 'Top-K',
      ...
      html4excel: '表格轉HTML',
      html4excelTip: `與 General 切片方法配合使用。未開啟狀態下，表格檔案（XLSX、XLS（Excel 97-2003）會按行解析為鍵值對。開啟後，表格檔案會被解析為 HTML 表格。若原始表格超過 12 行，系統會自動按每 12 行拆分為多個 HTML 表格。欲了解更多資訊，請參閱 https://Panython.io/docs/dev/enable_excel2html。`,
      autoKeywords: '自動關鍵字',
      autoKeywordsTip: `自動為每個文字區塊中提取 N 個關鍵詞，以提升查詢精度。請注意：此功能採用「系統模型設定」中設定的預設聊天模型提取關鍵詞，因此也會產生更多 Token 消耗。此外，你也可以手動更新生成的關鍵詞。詳情請參見 https://Panython.io/docs/dev/autokeyword_autoquestion。`,
      autoQuestions: '自動問題',
      autoQuestionsTip: `為了提高排名分數，請使用「系統模型設定」中定義的聊天模型，為每個知識庫區塊提取 N 個問題。 請注意：這會消耗額外的 token。 結果可在區塊列表中查看和編輯。 問題提取錯誤不會阻止分塊過程； 空結果將被添加到原始區塊。詳情請參見 https://Panython.io/docs/dev/autokeyword_autoquestion。 `,
...
`,
    ...
    <p>
      ...
      resume: `<p>支持的文件格式為<b>DOCX</b>、<b>PDF</b>、<b>TXT</b>。
        ...
        您只需與<i>'Panython'</i>交談即可列出所有符合資格的候選人。
        </p>
          `,
...
`,
      ...
      useRaptorTip:
        '啟用 RAPTOR 以用於多跳問答任務。詳情請參見：https://Panython.io/docs/dev/enable_raptor',
      prompt: '提示詞',
...
以上就是你需要總結的內容。`,
      ...
      entityTypes: '實體類型',
      pageRank: '頁面排名',
      pageRankTip: `知識庫檢索時，你可以為特定知識庫設置較高的 PageRank 分數，該知識庫中匹配文本塊的混合相似度得分會自動疊加 PageRank 分數，從而提升排序權重。詳見 https://Panython.io/docs/dev/set_page_rank。`,
      tagName: '標籤',
      frequency: '頻次',
      searchTags: '搜尋標籤',
      tagCloud: '雲端',
      ...
      <p>詳情請參閱 https://Panython.io/docs/dev/use_tag_sets。</p>
 `,
      tags: '標籤',
      addTag: '增加標籤',
      ...
      useGraphRag: '提取知識圖譜',
      useGraphRagTip:
        '基於知識庫內所有切好的文本塊構建知識圖譜，用以提升多跳和複雜問題回答的正確率。請注意：構建知識圖譜將消耗大量 token 和時間。詳見 https://Panython.io/docs/dev/construct_knowledge_graph。',
      graphRagMethod: '方法',
 ...
 NER：使用 spaCy NER 和基於規則的關鍵詞提取來抽取實體和關係，無需 LLM 參與提取過程，速度快且資源消耗低`,
    ...
    chat: {
      ...
      createChatAssistant: '創建聊天助手',
      historySessionCount: '共 {{count}} 條歷史會話',
      startChat: '發起聊天',
      newConversation: '新會話',
     ...
     「系統提示詞」中的所有變數都必須用大括號{}括起來。詳見 https://Panython.io/docs/dev/set_chat_variables。`,
      add: '新增',
      key: '關鍵字',
      optional: '可選的',
      operation: '操作',
    ...
    setting: {
      ...
      ttsModel: '語音合成模型',
      ttsModelTip:
        '默認的tts模型會被用於在對話過程中請求語音生成時使用。如未显示可选模型，请根据 https://Panython.io/docs/dev/supported_models 确认你的模型供应商是否提供该模型。',
      embeddingModel: '嵌入模型',
      embeddingModelTip:
        '如未顯示可選模型，請檢查你是否在使用 Panython slim 版（不含嵌入模型）；或根據 https://Panython.io/docs/dev/supported_models 確認你的模型供應商是否提供該模型。',
      img2txtModel: 'img2Txt模型',
      img2txtModelTip:
        '所有新創建的知識庫都將使用默認的 img2txt 模型。它可以描述圖片或視頻。如未顯示可選模型，請根據 https://Panython.io/docs/dev/supported_models 確認你的模型供應商是否提供該模型。',
      sequence2txtModel: 'speech2Txt模型',
      sequence2txtModelTip:
        '所有新創建的知識庫都將使用默認的 ASR 模型。使用此模型將語音翻譯為相應的文本。如未顯示可選模型，請根據 https://Panython.io/docs/dev/supported_models 確認你的模型供應商是否提供該模型。',
      rerankModel: 'rerank模型',
      rerankModelTip: `默認的 reranking 模型。如未顯示可選模型，請根據 https://Panython.io/docs/dev/supported_models 確認你的模型供應商是否提供該模型。`,
      workspace: '工作空間',
      upgrade: '升級',
      addLlmTitle: '添加Llm',
      modelName: '模型名稱',
...
例如，在長文本翻譯迭代節點內，如果所有內容都輸入到LLM節點，則可能會達到單次對話限制。上游節點可以先將長文本拆分為多個分片，並配合迭代節點對每個分片進行批次翻譯，避免達到單次對話的LLM訊息限制。`,
    ...
    search: {
      ...
      copySuccess: '複製成功',
      welcomeBack: '歡迎回來',
      resultsSummary: '找到 {{docs}} 個文檔，包含 {{chunks}} 個命中片段',
      hitChunks: '{{count}} 個片段',
      openDocument: '打開文檔',
